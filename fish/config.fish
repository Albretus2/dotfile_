...
# set fish_greeting "Selamat datang di mesin Linux" 
# $figlet -f fuzzy fish
#  .--. _       .-.
# : .-':_;      : :
# : `; .-. .--. : `-.
...
function fish_greeting -d "Theo's Custom Greetin Msg"
    ...
    ' \
        '
    You found the only "fish" that Oyen could not eat!
    ...
    set -l timestamp  $(date +"%T")
    set -l uptime $(uptime | grep -ohe 'up .*' | sed 's/,//g' | awk '{ print $2" "$3 " " }')

    # Print the msg
    echo
    echo -e "  " "$brgreen" "Welcome back Alberr 🐱" "$normal"
